...
public class ListAdapter extends RecyclerView.Adapter<ListHolder> {
    ...
    SortedList<UserSongRelation> items = new SortedList<>(UserSongRelation.class, new ListSorted<UserSongRelation>(this));
    ...
    public void onBindViewHolder(@NonNull ListHolder holder, final int position) {
        if (items.get(position) != null) {
            ...
            if (ListSorted.customOrder() && relation.join.getPosition() != position) {
                relation.join.setPosition(position);
                new UserSongUpdate(context, relation.join);
            }
